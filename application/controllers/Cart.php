...
class Cart extends CI_Controller
{
	...
	{
		$count = count($this->input->post('paket'));
		for ($i = 0; $i < $count; $i++) {
			$data_detail[$i] = array(
	...
	public function getJamMulai()
	{
		...
		if ($tanggal === FALSE || $paket_id === FALSE) {
			echo json_encode(array());
			die();
		}

		$list_jam_mulai_terpakai = $this->Transaksi_detail_model->get_jam_mulai_terpakai($tanggal, $paket_id);
